...
Un solo disparador activo (septiembre 2026), más dos en reserva:
  ...
  · GitHub Actions schedule   :07 :22 :37 :52  → ingest_purpleair.py     [pausado a mano, ver "Historia" abajo]
...
| `.github/workflows/purpleair-ingest.yml` (`schedule:`) | GitHub Actions, minutos :07/:22/:37/:52. Corre `ingest_purpleair.py`, misma lógica en Python, escribiendo a D1 vía su API HTTP. **Estado (septiembre 2026): pausado** (el `schedule:` queda comentado en el workflow, no borrado) para no duplicar el gasto de puntos de la API junto con cron-job.org — ver "Pausa de GitHub Actions..." arriba. `workflow_dispatch` sigue disponible para correrlo a mano. Mientras estuvo activo, tenía delays variables e inconsistentes (documentado por GitHub como comportamiento esperado bajo carga — en la práctica, huecos de 30-55 min en vez de 15). |
